...
srre.Exroll=function(values){
    ...
    sendChat('RollEx','[['+values.nDice+'d6cs>5>5sd]]',function(result){
        ...
        if(hitCount>values.limit&&values.limit>0){
            numb.successes+=parseInt(values.limit);
        }else{
        ...
        values.nSuccesses+=numb.successes
        values.numb.push(numb)
        if(critGlitch){
            sendout={}
            sendout.limit=values.limit
    ...
    if(values.numb){
        ...
        for(var i=0;i<values.numb.length;i++){
            ...
            if(values.numb[i].rolls){
                ...
                if(values.limit>0){
                    showing+='kh'+values.limit
                }
                showing+='= ('
                for(var j=0;j<values.numb[i].rolls.length;j++){
                    if(values.numb[i].rolls[j].v>=5&&((values.limit>0&&j<=values.limit)||values.limit==0)){
                        showing+='<span style='+'&'+'quot'+';'+'font-size:1.1em;color:#00ff00;'+'&'+'quot'+';'+'>'
                    }else if(j>values.limit&&values.limit>0){
                        showing+='<span style='+'&'+'quot'+';'+'font-size:1.1em;color:#777777;'+'&'+'quot'+';'+'>'
...
srre.Rollextender=function(msg){
    if(msg.content.match(/RollEx/i)!=null){
        ...
        if(temp){
            ...
            var limit=msg.content.match(/l\d+/i)
            var arg={}
